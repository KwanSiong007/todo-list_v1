...
export default function Home() {
  //The useState function takes an initial state value as an argument. In this case, it uses an inline arrow function (() => { ... }) to set the initial state based on the value retrieved from the localStorage.
  const [todos, setTodos] = useState(() => {
    ...
    const localValue = localStorage.getItem("SECOND-ITEM");
  ...
  useEffect(() => {
    localStorage.setItem("SECOND-ITEM", JSON.stringify(todos));
  }, [todos]);

  //value of `newItem` in NewTodoForm component passed as an argument to the addTodo function, and it becomes the value of the `title` parameter.
  ...
  const [theme, setTheme] = useState(() => {
    //we  get a value that was stored under the key "theme-home" in localStorage.
    const storedTheme = localStorage.getItem("theme-home");
  ...
  const toggleTheme = () => {
    ...
    //It sets a key-value pair in localStorage with the key "theme-home" and the value of newTheme.
    localStorage.setItem("theme-home", newTheme);
  };
